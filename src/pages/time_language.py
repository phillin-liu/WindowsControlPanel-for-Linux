...
def _timezones():
    # 优先用 Python 标准库 zoneinfo（不启动子进程，避免 timedatectl 偶发挂起导致界面假死）
    try:
        ...
        pass
    ok, out, _ = utils.run(["timedatectl", "list-timezones"], timeout=5)
...
class TimeLanguagePage(BasePage):
    ...
    def _set_ntp(self, on: bool):
        ...
        if not ok:
            # 认证被取消或失败：回滚开关
            self.ntp_toggle.set_checked_silent(not on)
