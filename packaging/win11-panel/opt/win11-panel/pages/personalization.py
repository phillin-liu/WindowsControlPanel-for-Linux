...
class PersonalizationPage(BasePage):
    ...
    def _build(self):
        ...
        # 跟随系统按钮
        follow_btn = QPushButton("跟随系统")
        follow_btn.setToolTip("使用当前系统桌面环境的强调色 (GNOME/KDE)")
        follow_btn.clicked.connect(self._follow_system_accent)
        accent_row.addWidget(follow_btn)
    ...
    def _follow_system_accent(self):
        """清除用户自定义, 改用系统主题色"""
        s = QSettings("Win11Panel", "Win11Panel")
        s.remove("theme/accent")
        sys_accent = theme.detect_system_accent() or "#0067C0"
        # 临时保存 (避免又读回旧值), 但标记是系统跟随
        s.setValue("theme/_follow_accent", True)
        s.setValue("theme/accent", sys_accent)
        app = QApplication.instance()
        theme.apply_theme(app)
        win = self.window()
        if hasattr(win, "refresh_nav_icons"):
