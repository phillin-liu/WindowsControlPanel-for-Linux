...
class UpdatePage(BasePage):
    def __init__(self):
        super().__init__("软件更新", "查看系统版本与更新分支，检查并安装系统更新。")
        self._cmd = None
        self._build()
    ...
    def _build(self):
        # 当前系统分支
        rel = _os_release()
        c0 = self.add_card("当前系统")
        pretty = rel.get("PRETTY_NAME", "")
        distro = rel.get("NAME", "Linux")
        branch = rel.get("VERSION_CODENAME", "") or rel.get("VERSION_ID", "")
        sys_row = SettingRow("系统版本", pretty or "未知", icon="system")
        c0.add(sys_row)
        if branch:
            c0.add(SettingRow("更新分支",
                              f"{distro} {branch}", icon="update"))
        # 内核版本
        ok, kver, _ = utils.run(["uname", "-r"], timeout=5)
        if ok:
            c0.add(SettingRow("内核版本", kver, icon="usb"))
        ...
        self.log.setPlaceholderText("更新过程将显示在这里…")
    ...
    def _refresh_history(self):
        ...
        if not text:
            self.history_label.setText("暂无更新历史记录。")
            return
        try:
            ...
            self.history_label.setText("无法读取更新历史。")
