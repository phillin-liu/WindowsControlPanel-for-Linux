    ...
    def _uninstall(self):
        ...
        if pkg:
            if confirm_box(self, "卸载应用",
                           f"将执行: sudo apt purge -y {pkg}\n确定卸载 {self.entry['title']} 吗？"):
                ok, _o, err = utils.run_root(["apt-get", "purge", "-y", pkg])
                info_box(self, "完成" if ok else "失败",
                         "应用已卸载。" if ok else err)
        elif utils.have("flatpak"):
            ok, _o, _e = utils.run(["flatpak", "list", "--app", "--columns=application"],
                                   timeout=10)
            if ok and self.entry["appid"] in _e + _o:
                if confirm_box(self, "卸载 Flatpak 应用",
                               f"flatpak uninstall -y {self.entry['appid']}"):
                    utils.run_root(["flatpak", "uninstall", "-y", self.entry["appid"]])
                    info_box(self, "完成", "应用已卸载。")
        else:
