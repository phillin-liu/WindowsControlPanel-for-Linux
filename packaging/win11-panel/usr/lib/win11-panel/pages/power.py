...
class PowerPage(BasePage):
    ...
    def on_show(self):
        ...
        # GNOME 空闲值回显
        v = utils.gget("org.gnome.desktop.session", "idle-delay")
        if v:
            try:
                sec = int(str(v).replace("uint32 ", ""))
                i = self.screen_off.findData(sec)
                if i >= 0:
                    self.screen_off.setCurrentIndex(i)
            except ValueError:
                pass
        v2 = utils.gget("org.gnome.settings-daemon.plugins.power",
                        "sleep-inactive-ac-timeout")
        if v2:
            try:
                sec = int(str(v2).replace("uint32 ", ""))
                i = self.sleep_combo.findData(sec)
                if i >= 0:
                    self.sleep_combo.setCurrentIndex(i)
            except ValueError:
                pass
