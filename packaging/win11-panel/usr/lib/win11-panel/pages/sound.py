...
def _muted() -> bool:
    ...
    return ok and "是" in out.lower() or ok and "yes" in out.lower()
...
class SoundPage(BasePage):
    ...
    def _build(self):
        ...
        if not utils.have("pactl"):
            tip = QLabel("未检测到 pactl (pulseaudio-utils)。请安装后使用声音功能："
    ...
    def refresh(self):
        ...
        self.mute_toggle.setChecked(_muted(), animate=False)
        cur = _default_sink()
