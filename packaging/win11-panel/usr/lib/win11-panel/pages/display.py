...
class DisplayPage(BasePage):
    ...
    def refresh(self):
        ...
        # 缩放
        if utils.session_type() != "wayland" and outputs:
            cur = utils.gget("org.gnome.desktop.interface", "text-scaling-factor", "1.0")
    ...
    def _refresh_brightness(self):
        max_f = min_f = None
        for d in os.listdir("/sys/class/backlight") if os.path.isdir("/sys/class/backlight") else []:
            base = f"/sys/class/backlight/{d}"
            try:
