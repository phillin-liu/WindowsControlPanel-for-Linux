...
class NetworkPage(BasePage):
    ...
    def _toggle_wifi(self, on: bool):
        utils.run(["nmcli", "radio", "wifi", "on" if on else "off"], timeout=5)
        if on:
            utils.run_root(["rfkill", "unblock", "wifi"])
    ...
    def _toggle_airplane(self, on: bool):
        ...
        ok, _o, err = utils.run(["nmcli", "radio", "all", "off" if on else "on"],
                                timeout=5)
        if ok and on:
            utils.run_root(["rfkill", "block", "bluetooth"])
        elif not on:
            utils.run_root(["rfkill", "unblock", "wifi"])
        if not ok:
    ...
    def on_show(self):
        if not utils.have("nmcli"):
            self.status_label.setText("未检测到 NetworkManager (nmcli)。"
                                      "请安装：sudo apt install network-manager")
            return
        # 状态
        ...
        # Wi-Fi
        self.wifi_toggle.setChecked(_wifi_enabled(), animate=False)
        self._rebuild_wifi()
        # 以太网
