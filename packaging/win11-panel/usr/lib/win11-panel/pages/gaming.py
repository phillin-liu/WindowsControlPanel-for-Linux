...
class GamingPage(BasePage):
    ...
    def _detect_platforms(self):
        ...
        for name, cmds in PLATFORMS:
            ...
            if installed:
                ...
                lay.addWidget(row)
        if not found_any:
            lay.addWidget(QLabel("未检测到游戏平台。可安装 Steam: sudo apt install steam "
                                 "或使用 lutris/wine 运行 Windows 游戏。"))
    ...
    def on_show(self):
        self.gm_toggle.setChecked(self._game_mode_on(), animate=False)
        ...
        self._shot_label.setText(
            ...
            "未安装截图工具，推荐: sudo apt install flameshot")
